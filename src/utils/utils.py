...
def init_seed(seed, deterministic=True):
    ...
    When deterministic=True (default), also fix cuDNN/CUDA to get same results across runs with same seed.
    """
    random.seed(seed)
    np.random.seed(seed)
    ...
    if deterministic:
        ...
        # PyTorch 1.8+: optional; set before first CUDA op if you need deterministic matmul etc.
        # os.environ['CUBLAS_WORKSPACE_CONFIG'] = ':4096:8'  # and torch.use_deterministic_algorithms(True)
